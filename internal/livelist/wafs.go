/*
 * Двоичные объекты набора keeper (keeper/internal/wire/pack.go): снапшот и
 * пакет изменений. Раскладка -- little-endian, фиксированная:
 *
 *   заголовок, 52 байта: "WAFS", версия u8, вид u8 (1 снапшот, 2 пакет),
 *   тип u8 (1 cidr, 2 string), флаги u8 (бит 0 -- есть повод), epoch u64,
 *   seq u64, hash u64, ключ эпохи 16 байт, count u32
 *
 *   запись cidr:   op u8 | семейство u8 | биты u8 | адрес 4|16 | срок u64
 *   запись string: op u8 | длина u16 | байты | срок u64 | [длина u16 | повод]
 *
 * Хеш записи -- SipHash-2-4 под ключом эпохи по её материалу: у адреса
 * семейство‖биты‖адрес (те же байты, что в записи), у строки -- она сама.
 * Кодировщик здесь ради тестов: в работе зеркало только читает.
 */
...
// material -- байты хеша записи.
func (r packRecord) material(typ uint8) []byte {
	if typ == typeCIDR {
		return prefixMaterial(r.prefix)
	}
...
// pack кодирует объект как keeper; снапшот keeper сортирует, здесь порядок
// вызывающего.
func pack(head packHead, recs []packRecord) []byte {
	head.count = uint32(len(recs))

	out := make([]byte, 0, packHeader+len(recs)*32)
	out = append(out, packMagic...)
	out = append(out, packVersion, head.kind, head.typ, head.flags)
	...
	for _, r := range recs {
		...
		if head.typ == typeCIDR {
			out = append(out, prefixMaterial(r.prefix)...)
