...
| Redis: внутренний | для проверки по списку | зеркало динамических списков и тела статических, по которым правила проверяют значение куки; без него берётся буфер |
...
машине и уходит в дампы. Если заданы оба, файл сильнее.

**Смена ключа делает все выданные куки поддельными.** Каждая кука, подписанная прежним ключом,
придёт как `invalid`, и если профиль начисляет за `invalid` баллы, их разом получат все вернувшиеся
...
| `REDIS_INTERNAL_URL` | из `inspector.conf` | внутренний Redis: зеркало динамических списков, тела статических |
